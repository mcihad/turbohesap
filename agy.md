...
| product role ↔ capability-flag mapping (form preset, no DB column) | `shared/src/modules/inventory/product-role.ts` (`PRODUCT_ROLE_PRESETS`, `inferProductRole`) — one source reused by web `product-form-dialog.tsx` + mobile `ProductFormScreen.tsx`; flags enforced server-side in invoices/orders/pos (canBeSold/canBePurchased) + production create (canBeManufactured); production `complete()` skips movements for `trackStock=false` items |
| recipe (reçete) — a sold item's ingredients backflushed silently at sale | `backend/src/modules/inventory/recipes.service.ts` `consume(em, {...}) → warnings` — reused by POS `settleInTx` + sales-invoice `postInvoiceStock` (both `import InventoryModule`); posts `'Reçete Sarf'` OUT at AVCO, skips `trackStock=false` ingredients, warn-but-continue via transient `stockWarnings` on `PosOrderDto`/`InvoiceDto`. Editor: product-detail "Reçete" tab (web) / POS section (mobile). Distinct from bundles (visible priced lines) and BOM (production-only) |
| atomic sequential-number generator (race-free, vs. the racy `count()+1`/MAX+1 patterns in invoices/orders/production/contacts/pos/assets numbering) | `backend/src/modules/lookups/code-prefixes.service.ts` `consume()` — single `UPDATE ... SET next_number = next_number + 1 ... RETURNING`, no lock/transaction needed; web `frontend/src/components/code-prefix/{use-code-prefix.ts,code-prefix-input.tsx}`, mobile `mobile/src/lib/use-code-prefix.ts` + `mobile/src/components/CodePrefixInput.tsx`. Fatura/irsaliye numbers are intentionally NOT wired to this — see `docs/lookups.md` §7's gapless-numbering callout |
...
| **PostGIS geofence + spatial geometry** (giriş alanları) | enable the extension in a migration (`CREATE EXTENSION IF NOT EXISTS postgis`, first statement — like `uuid-ossp` in `Init`). Geometry columns are **always named `geom`**, `@Column('geometry', { srid: 4326, spatialFeatureType? })`; TypeORM round-trips them as **GeoJSON**. Add GIST indexes by hand in the migration (`CREATE INDEX … USING GIST ("geom")` — `migration:generate` won't). Distance/containment in **meters** = `ST_DWithin(geom::geography, ST_SetSRID(ST_MakePoint($lng,$lat),4326)::geography, meters)` via a raw `repo.query(...)`. Reference: `backend/src/modules/hr/{entities/checkin-area.entity.ts, attendance.service.ts}`, migration `…/migrations/*-AddHrPdks.ts` |
| **OpenLayers map / geometry editor** (web) | `frontend/src/modules/hr/components/map/geometry-editor.tsx` (`ol` dep). Round-trip geometry as GeoJSON: `new GeoJSON().readFeatures(gj,{dataProjection:'EPSG:4326',featureProjection:'EPSG:3857'})` / `writeFeatureObject(...)`. `ol/interaction/{Draw,Modify,Snap}` for polygon/point editing; OSM base layer. First map UI in the codebase |
| **react-native-maps geometry editor** (mobile) | `mobile/src/modules/hr/components/map/AreaGeometryEditor.tsx` (`react-native-maps`, native dep → prebuild; Android Google key placeholder in `app.json` `android.config.googleMaps.apiKey`, optional). **OSM is the visible basemap** via `<UrlTile urlTemplate="https://tile.openstreetmap.org/{z}/{x}/{y}.png"/>` + `mapType="none"` + `PROVIDER_DEFAULT` (no Google key needed at iOS runtime). Tap-to-add polygon vertices + draggable `<Marker>`s, point mode = draggable center + `<Circle radius>`; convert map `{latitude,longitude}` ↔ GeoJSON `[lon,lat]` (close/strip the polygon ring). `expo-location` "use my location". Full PDKS admin set is mirrored on mobile (shifts/rotations/schedule/areas/attendance/cards) — dedicated FormScreens, not modals |
| **expo-location mobile check-in** (geofence clock-in) | `mobile/src/modules/hr/CheckinScreen.tsx` — foreground permission (mirror stocktake camera), `Location.getCurrentPositionAsync` → `api.hr.attendance.checkin({direction,lat,lng,accuracyMeters,isMockLocation})`; server returns accepted/flagged + message. Native dep → `expo prebuild` + location permission strings in app config |
| shift (vardiya) system: definitions + rotation + materialized calendar | `backend/src/modules/hr/{shifts,shift-rotations,shift-schedule}.service.ts`; rotation→day math in pure `hr-pdks.helpers.ts` (`rotationSlotIndex`); `generate` materializes `hr_employee_shift_days` (UNIQUE employee+date, manual overrides preserved) |
| card-access (PDKS) import standard (vendor-neutral) | `backend/src/modules/hr/attendance.service.ts` `import()` (dedup by `(source,externalId)` partial-unique + app pre-check; cardNo→employee resolution chain); shared `card.dto.ts` (`AccessEvent`/`AttendanceImportRequest`); documented in `docs/hr-pdks.md` + the in-app "Veri Standardı" tab |
| fixed-asset registry + custody (zimmet) + **barcode transfer handshake** | shared `shared/src/modules/inventory/asset*.{dto,service,client}.ts`; backend `backend/src/modules/inventory/asset-custody.service.ts` (transfer state machine: initiate→byToken→accept in one `manager.transaction`, closes giver's active assignment + opens receiver's + flips `Asset.currentEmployee*`), `assets.service.ts` (lifecycle `changeStatus` auto-closes zimmet), `asset-maintenance`/`asset-vehicle-logs.service.ts`; cross-module `Employee` borrowed into inventory `forFeature` (HR exports nothing); mobile star `mobile/src/modules/inventory/TransferReceiveScreen.tsx` (expo-camera QR scan → `byToken` → `accept`) + `TransferInitiateScreen.tsx` (`react-native-qrcode-svg`). NOT stock — independent `inventory_assets`. Perms `inventory.assets.{read,write,assign,maintain}` |
| **manufacturing / MRP (the whole thing)** | `docs/production.md` + `backend/src/modules/production/*`. Copy `manufacturing-orders.service.ts` for a document that explodes a BOM (phantom-recursive `explodeComponent`), snapshots children, reserves, then in `complete()` consumes+produces+rolls up cost in one `manager.transaction`. `production.module.ts` imports `InventoryModule` and injects its `StockMovementsService`/`CostService`/`ReservationsService`; InventoryModule reads `ProductionOrder`+`OrderDocument(Line)` **read-only in forFeature** for ATP (`orders→production` hard import forbidden — MTO is pull). Perms `production.{read,write,orders.*,workorders.execute,subcontract.manage,planning.run,quality.manage}` |
| **AVCO moving-average cost** | `backend/src/modules/inventory/{cost.service.ts (applyMovingAverage), entities/product-cost.entity.ts}`; hook lives in `stock-movements.service.ts post()` — on `direction==='in'` with `unitCost`, weight by on-hand captured BEFORE `adjustOnHand`. `out` values at current avg, never mutates it. `PostMovementParams.unitCost` + `CreateStockMovementRequest.unitCost` feed it |
| **stock reservation + ATP** | `backend/src/modules/inventory/{reservations.service.ts (reserveTxn/releaseSourceTxn finalStatus), availability.service.ts}`; `ProductStock.reservedQty` (NOT a ledger row) → `available = quantity − reservedQty`; `atp = available + incoming(open MO output + open purchase OrderDocument)`. Shared `stock-ops.{dto,service,client}.ts`, routes `/inventory/{reservations,cost,availability}` |
| **MRP planning (net + multi-level explode + apply→MO)** | `backend/src/modules/production/planning.service.ts` — `addDemand()` nets each item vs ATP (cache), records net req, recurses into its active BOM for dependent demand; `apply()` turns manufacture suggestions into draft MOs. Reorder = order-up-to-max when `available<min`. MTO = `manufacturing-orders.service.ts createFromDemand` (+ `/orders/from-demand`) |
| **lot/serial traceability (genealogy + recall)** | `backend/src/modules/production/lots.service.ts` — `production_lots` (unique product+lotNo) + `production_order_lots` (role consumed\|produced); `trace(lotId)` walks both directions (producedFrom→upstream consumed, consumedInto→downstream produced). Quality gate: `work-orders.service.ts finish()` blocks when `qualityCheckRequired && !hasPassingCheck` |
| **decimalTransformer + DB default gotcha** | `decimalTransformer.to(undefined) → null` overrides the column DEFAULT → **not-null violation**. In every `em.create(Entity, {…})` you MUST explicitly set each `numeric`-with-default field to `0` (e.g. all MO cost fields, WO produced/rejected/actualMinutes) — do not rely on the DB default. Seen in `manufacturing-orders.service.ts`. Also bit twice in `financial-instruments.service.ts`'s `postSettlement()` — forgot `ContactTransaction.exchangeRate` (decimalTransformer, DB default 1) → 500 at runtime; every transformer column on a row you `em.getRepository(X).create({…})` needs an explicit value, not just the ones that look money-like |
| **general-purpose document management (Evrak) + real server-enforced privacy** | `docs/documents.md` + `backend/src/modules/documents/*`. Category tree is a byte-for-byte **duplicate** of `inventory/categories.service.ts` (adjacency list, cycle guard, delete-with-children block) — plus `isPrivate`/`ownerId` privacy enforced **in the query itself** (`documents.service.ts`/`document-categories.service.ts` `findVisibleOrFail`: `WHERE isPrivate=false OR ownerId=:userId` unless caller holds `documents.private.readAll`; `get()` 404s, never 403, on a row you can't see). Shared helper `privacy.util.ts` (`hasPrivateReadAll`/`requirePrivateManage`/`privacyChangeNeedsManage`) — the first genuine row-level ACL in the codebase (everything else is permission-key-only). `DocumentsModule` imports `IamModule` for `AccessService` and reads `files`' `FileEntity` read-only (same precedent as `feedback.module.ts`) for per-document attachment counts. Süreli-evrak status (`expiryStatus`) is **computed at read time** from `expiryDate`, never stored — mirrors finance balances; no cron/notification system was added (out of scope, see doc) |
| **Çek/Senet (checks/promissory notes) built on top of a generic module** | `backend/src/modules/finance/financial-instruments.service.ts`. One entity (`FinancialInstrument`) discriminated by `instrumentType`/`direction` (the `orders` `OrderDocument` kind/direction pattern) instead of two near-duplicate entities. `create()` auto-opens a linked `documents.Document` (idempotent system category `code='CEK_SENET'`, same find-or-create idiom as `StockMovementTypesService.systemTypeId`); `remove()` (only from `status='open'`) unlinks the document (`relatedEntityType/Id → null`) but never deletes it. `collect`/`pay`/`reverse` copy `invoices.service.ts`'s `addPayment`/`removePayment` **line for line**: one `manager.transaction`, a `FinanceTransaction` + a `ContactTransaction` created inside it, their ids stored back on the owning row (`financeTransactionId`/`contactTransactionId`) so reversal is a delete-by-stored-id, never a search. `FinanceModule` imports `DocumentsModule` (one-way — `documents` never imports `finance`) |
