...
/**
 * Create/edit dialog for a category's CORE fields + gizlilik (privacy). Saves
 * only on "Kaydet". Custom field definitions, sub-categories, evraklar and audit
 * live on the category **detail page** — this dialog stays focused, mirroring
 * `inventory/components/category-dialog.tsx`.
 */
export function CategoryDialog({
  ...
  // A caller can always manage privacy on a row they own (or a brand-new one);
  // otherwise `documents.private.manage` is required — mirrors the server's
  // `privacyChangeNeedsManage` check exactly (see backend `privacy.util.ts`).
  ...
  const save = useMutation({
    mutationFn: () => {
      const payload = {
        ...
        // Only send an explicit owner when the caller can actually reassign
        // ownership — otherwise the server keeps/defaults it (self on create).
        ...(canManagePrivacy ? { ownerId: form.ownerId } : {}),
      }
      return editing
        ? api.documents.categories.update(editing.id, payload)
        : api.documents.categories.create(payload)
    },
    onSuccess: (saved) => {
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            {editing
              ...
              : 'Yeni bir evrak kategorisi oluşturun. Özel alanlar detay sayfasından eklenir.'}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid max-h-[65vh] gap-4 overflow-y-auto px-1 py-2">
          ...
          <div className="space-y-2 rounded-lg border p-3">
            ...
            {!privacyEditable ? (
              <p className="text-xs text-muted-foreground">
                Bu kategori başka birine ait — gizlilik ayarlarını değiştirmek için “gizlilik yönetimi” yetkisi gerekir.
              </p>
            ) : null}
          </div>
        </div>
