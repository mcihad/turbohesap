// FieldDefBuilder — manage a category's custom product-field schema
// (CategoryFieldDef[]). Fields are shown as a compact list; **add/edit happens in
// a modal**. The `lookup` type picks its source list from a **combobox** of
// existing lookup lists.
...
const BLANK: CategoryFieldDef = { key: '', label: '', type: 'text', required: false }

export function FieldDefBuilder({
  value,
  onChange,
}: {
  value: CategoryFieldDef[]
  onChange: (defs: CategoryFieldDef[]) => void
}) {
  ...
  const upsert = (def: CategoryFieldDef) => {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <Label>Ürün alanları (bu kategorideki ürünlere özel)</Label>
      ...
      {value.length === 0 ? (
        <p className="rounded-lg border border-dashed p-4 text-center text-sm text-muted-foreground">
          Henüz özel alan yok. Bu kategorideki ürünlere girilecek ek alanları
          (beden, renk, tarih…) “Alan ekle” ile tanımlayın.
        </p>
      ) : (
...
function FieldDefDialog({
  ...
  open: boolean
  initial: CategoryFieldDef
  isNew: boolean
  onClose: () => void
  onSave: (def: CategoryFieldDef) => void
}) {
  const [def, setDef] = React.useState<CategoryFieldDef>(initial)
  ...
  const set = (patch: Partial<CategoryFieldDef>) => setDef((d) => ({ ...d, ...patch }))
    ...
    <Dialog open={open} onOpenChange={(o) => (o ? null : onClose())}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Bu kategorideki ürünlere girilecek özel bir alan tanımlayın.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid max-h-[60vh] gap-4 overflow-y-auto px-1 py-1">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              <Label>Tür</Label>
              <Select value={def.type} onValueChange={(v) => set({ type: v as CategoryFieldType })}>
          ...
          {def.type === 'lookup' ? (
            <div className="space-y-1.5">
              ...
              <p className="text-2xs text-muted-foreground">
                Ürün formunda bu listeden seçtirilir (LookupSelect).
              </p>
            </div>
          ) : null}
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  placeholder={def.type === 'money' ? 'TRY' : 'gr'}
                />
              </div>
            </div>
          ) : null}
