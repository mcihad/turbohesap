...
| `data-grid` | `DataGrid` — TanStack tablo; sırala/grupla/pin/gizle/sürükle + per-user kalıcı düzen. **`server={{total,onQueryChange}}}`** ile sunucu tarafı sayfalama/filtre/sıralama (varsa `data`=sadece o sayfa, `manual*` açılır, state'ten bir `ListQuery` türetilip debounce ile emit edilir); prop yoksa tam client-side (geri uyumlu). Tipli sütun filtreleri: kolona `meta.filter={{type:'text'\|'number'\|'date'\|'daterange'\|'boolean'\|'select'\|'lookup', field?, lookupList?, options?}}` — başlıkta stillendirilmiş operatör+değer popover'ı. `queryBuilder={{fields}}` ile iç içe VE/VEYA Query Builder açılır; sütun filtreleri + QB tek filtre ağacına AND ile birleşir ve grid state'te kalıcıdır |
| `query-builder` | `QueryBuilder` (`{fields, value, onChange}`) — bağımsız, yeniden kullanılabilir iç içe VE/VEYA filtre kurucu (`FilterGroup` üretir); `filter-value-editor` ile tipli değer editörleri (lookup/tarih/bool/select/…). DataGrid dışında da kullanılabilir |

**Sunucu tarafı grid altyapısı (referans):** shared filtre modeli `@turbohesap/shared` `core/list-query.ts` (`FilterOperator`, `FilterRule`, `FilterGroup`, `ListQuery`, `OPERATORS_BY_TYPE`, `encode/decodeListQuery`); backend motoru `backend/src/common/list/{apply-list-query.ts,list-query.dto.ts}` (allowlist'li kolonlar → enjeksiyon-güvenli, bind-param, `getManyAndCount` → `Page<T>`). Grid `list()` (dizi, picker'lar) DEĞİŞMEZ; sayfalı grid için ayrı `listPage()` + `GET .../paged`.

**Server moduna geçmiş gridler:** `contacts.contacts`, `invoices.list`, `inventory.products`, `inventory.stockMovements` (product-scoped). Diğer ~43 grid client modda çalışmaya devam eder — göç tarifi: shared `*ListQuery extends ListQuery` + `listPage→Page`, backend `applyListQuery`+`getManyAndCount`+`*-list-query.dto`, controller `@Get('paged')`, sayfa `useQuery([...,serverQuery])`+`data={page.items}`+`server={{total,onQueryChange}}`+kolon `meta.filter`+`queryBuilder`. (Kalanlar: contacts leads/opportunities/groups/transactions, finance ×3, hr ×8, iam ×3, documents ×3, inventory categories/assets/movementTypes/category.products/analysisMovements, lookups ×2, org.branches, production ×3, sales.channels, stocktake, feedback — hepsi backend'de çıplak `T[]` döndüğünden service+dto+client göçü gerekir.)
