// DynamicAttributeFields — renders a category's custom field definitions as live
// inputs, bound to a product's `attributes` bag. The input per type:
//   text/textarea/number/money/boolean/select/multiselect/date/daterange/lookup.
// Used by the product form: pick a category → its effective fields appear here.
...
// Re-export the shared validator under the name the product form imports.
export const missingRequired = missingRequiredAttributes
...
export function DynamicAttributeFields({
  ...
  /** Effective fields with their source category (effectiveFieldDefsWithSource). */
  ...
  const groups: { id: string; name: string; fields: CategoryFieldDef[] }[] = []
